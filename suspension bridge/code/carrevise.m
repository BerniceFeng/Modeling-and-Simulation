...
m_car = 100; %car mass %10 50 100

g=9.8; % gravity
num=10; % number of unit block
...
t_eq_start = 11; %starting time of the earthquake
t_eq_end = 14; %ending time of the earthquake
f_earth = 5; %frequency of the earthquake
a_earth = 0.001; %amplitude of the earthquake
...
% when the car passes
  ...
  if(t_eq_start < t) && (t < t_eq_end)
      ...
      U(2*a+3,1)=2*pi*cos(2*pi*(t-t_eq_start)*f_earth)*a_earth;
      U(2*a+4,1)=2*pi*cos(2*pi*(t-t_eq_start)*f_earth)*a_earth;
      U(2*a+2*b+5,1)=2*pi*cos(2*pi*(t-t_eq_start)*f_earth)*a_earth;
      U(2*a+2*b+6,1)=2*pi*cos(2*pi*(t-t_eq_start)*f_earth)*a_earth;
  end
